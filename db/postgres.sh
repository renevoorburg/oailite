...
db::get_some_ids() {
    ...
    local table=$2
    echo "\pset pager off
        SELECT id from ${database}.${table} WHERE true $restrict_sql ORDER BY timestamp OFFSET $offset LIMIT $rowcount ;" | ${DB_CLIENT} -t -A -q 2> /dev/null
}
...
db::get_record_data() {
    ...
    local table=$2
    echo "\pset pager off
        SELECT sourcedata from ${database}.${table} WHERE id='$record_id' ;" | ${DB_CLIENT} -t -A -q 2> /dev/null
}
...
db::get_number_of_records() {
    ...
    local table=$2
    echo "\pset pager off
        SELECT count(id) from ${database}.${table} WHERE true $restrict_sql ;" | ${DB_CLIENT} -t -A -q 2> /dev/null
}
